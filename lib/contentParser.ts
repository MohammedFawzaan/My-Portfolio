...
export function getPortfolioData(): PortfolioData {
  ...
  const result: any = {
    ...
    experience: [],
    codingProfiles: [],
    achievementsCertifications: { achievements: [], certifications: [] },
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    if (currentSection.includes('HERO + ABOUT')) {
      ...
      if (line.startsWith('Project ')) {
        ...
        currentProject = { title: extractVal(line, line.split(':')[0] + ':'), problem: '', techStack: [], description: '', keyFeatures: [], links: { github: '', live: '' } };
        inProjectArray = '';
      ...
      if (line.startsWith('Role:')) {
       ...
       if (line.includes('Prize') || line.includes('DSAThon') || line.includes('Talent Hunt')) {
         result.achievementsCertifications.achievements.push(line);
       } else if (line.includes('Sigma') || line.includes('Data Structures') || line.includes('Internship')) {
         result.achievementsCertifications.certifications.push(line);
       }
